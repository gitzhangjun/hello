package com.shop.food.food_order.dao;

import com.shop.food.food_order.entity.FoodOrder;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * (FoodOrder)表数据库访问层
 *
 * @author makejava
 * @since 2020-03-22 19:37:06
 */
@Mapper
public interface FoodOrderDao {

    /**
     * 通过ID查询单条数据
     *
     * @param orderId 主键
     * @return 实例对象
     */
    FoodOrder queryById(Long orderId);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<FoodOrder> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param foodOrder 实例对象
     * @return 对象列表
     */
    List<FoodOrder> queryAll(FoodOrder foodOrder);

    /**
     * 新增数据
     *
     * @param foodOrder 实例对象
     * @return 影响行数
     */
    int insert(FoodOrder foodOrder);

    /**
     * 修改数据
     *
     * @param foodOrder 实例对象
     * @return 影响行数
     */
    int update(FoodOrder foodOrder);

    /**
     * 通过主键删除数据
     *
     * @param orderId 主键
     * @return 影响行数
     */
    int deleteById(Long orderId);

}